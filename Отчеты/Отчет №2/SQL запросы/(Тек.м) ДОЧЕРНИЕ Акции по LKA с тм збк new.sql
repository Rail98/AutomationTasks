...
--select @firstDateOfMonthTwoMonthAgo  from co_material -- проверка даты
...
,'LKA' typ
, case when actio.action_name is not NULL  then ROW_NUMBER () OVER ( partition by actio.network_id ORDER BY actio.agreed_volume desc) else 0 end as row
, isnull(actio.TG,'')TG
, isnull(actio.TG_name,'')TG_name
, isnull(actio.priznak,'') priznak
, '=СЦЕПИТЬ(RC[-10];"S";RC[-8];"S";RC[-4])' kod1
, '=СЦЕПИТЬ(RC[-11];"S";RC[-9];"A";RC[1])' kod2 
...
, isnull(actio.agreed_volume,0)agreed_volume
, isnull(actio.pt_name,'')pt_name,isnull(sum(actio.cost),0)cost, isnull(sum(actio.amount),0)amount
...
, isnull(act.action_name,'')action_name
,act.promo
,act.action_id
,isnull(convert(char,act.shipment_start,104),'')shipment_start, isnull(convert(char,act.shipment_end,104),'')shipment_end, isnull(convert(char,act.holding_start,104),'')holding_start, isnull(convert(char,act.holding_end,104),'')holding_end
...
from 
(select r.pos, r.region_name, sn.id network_id, sn.legal_name, sn.name network
			,a.action_id
	...
	--and an.status_id in(4)
	
		
	group by r.pos, r.region_name, sn.id, sn.legal_name, sn.name
			, a.action_id, als.large_scale_share
			...
			, agr.group_id, agr.group_name
			, prod.material_id, cm.name, cmp.cg, t.tg, an.filling_date
			, apsh.comment
			, cmp.brand--------------
	...
	(select sn.id tnet_id, mt.material_id,prod.group_id
	...
	and cc.factory_id=1
	group by sn.id, mt.material_id,prod.group_id) sale on ac.network_id=sale.tnet_id and ac.group_id=sale.group_id and ac.material_id=sale.material_id
	...
	and c.factory_id=1 and c.del=0
	and cc.factory_id=1
	group by sn.id, mt.material_id,prod.group_id) sale_pred on ac.network_id=sale_pred.tnet_id and ac.group_id=sale_pred.group_id and ac.material_id=sale_pred.material_id
	--where pos in (5)
	
	group by ac.pos, ac.region_name, ac.network_id, ac.legal_name,ac.network
							, ac.action_name,ac.promo,ac.action_id,ac.shipment_start, ac.shipment_end, ac.holding_start, ac.holding_end, ac.class_id, ac.class, ac.format_id, ac.format
							, ac.bonus_rf, ac.discount, ac.boxes, ac.one_to_one, ac.order_amount, ac.order_sum, ac.compensation_type
							, ac.status_id, ac.status, ac.comment
							, ac.agreed_volume
							, ac.pt_id, ac.pt_name, ac.cost, ac.amount, ac.cg, ac.tg, ac.filling_date, ac.priznak) act
							
							group by act.pos, act.region_name, act.network_id, act.legal_name,act.network
							, act.action_name,act.promo,act.action_id,act.shipment_start, act.shipment_end, act.holding_start, act.holding_end
							, act.status_id, act.status, act.status_comment, act.pt_name, act.TG, act.TG_name, act.filling_date, act.priznak) actio
	
	group by actio.pos, actio.region_name, actio.network_id, actio.legal_name,actio.network
							, actio.action_name, actio.promo,actio.action_id,actio.shipment_start, actio.shipment_end, actio.holding_start, actio.holding_end,actio.TG, actio.TG_name,actio.priznak, actio.filling_date
							,actio.agreed_volume
							...
							, actio.status_id, actio.status, case when status_id in (4) then actio.status_comment end 
							--, actio.pt_id
							, actio.pt_name
							,status_comment
...
left join sam_network_attr_value snv3 on snv3.network_id=sn.id and snv3.network_attr_id in (31,32)
...
and sn.id not in (select sn.id
	...
	and an.status_id not in (1)
	--and an.status_id in(4)
	--and snv3.network_attr_id=32
	group by sn.id)
GROUP BY r.pos
,r.region_name
,sn.id 
,sn.legal_name
,sn.name,case when snv3.network_attr_id=31 then 'DLKA'else  'LKA'  end	
		
							
	order by 1,3,16 desc